...
float ExtendViewport::get_screen_scale_worldspace() const {
  // Calculate pixels per world unit
  float x_scale = last_screen_width / (width * zoom);
  float y_scale = last_screen_height / (height * zoom);
  return std::min(x_scale, y_scale);
}

float ExtendViewport::get_screen_scale_screenspace() const {
  // Calculate pixels per screen unit
  float x_scale = last_screen_width / width;
  float y_scale = last_screen_height / height;
  return std::min(x_scale, y_scale);
}
